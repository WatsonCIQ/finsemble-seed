(() => {
	...
	const FEA = FEA_PATH_EXISTS
		...
		: undefined;
	const FEAPackager = FEA_PATH_EXISTS
		? require("@chartiq/finsemble-electron-adapter/deploy/deploymentHelpers")
		: undefined;
	...
	const taskMethods = {
		...
		includeFinsembleFolder: done => {
			fse
				.copy("./finsemble", "./dist/finsemble")
				.then(() => {
					console.log("success!");
					done();
				})
		...
		"build:prod": done => {
			async.series(
				[
					...
					taskMethods.build,
					taskMethods.includeFinsembleFolder
				],
				done
			);
		},
